 ...
 * @internal
 */
function getMembers(queryType, query, dbData, aliasToMemberNameMap) {
    const members = {};
    if (!dbData.length) {
        return members;
    }
    ...
    columns.forEach((column) => {
        ...
        if (path.length === 3 &&
            ((_a = query.dimensions) === null || _a === void 0 ? void 0 : _a.indexOf(calcMember)) === -1) {
...
function getCompactRow(membersToAliasMap, annotation, queryType, members, timeDimensions, dbRow) {
    ...
    members.forEach((m) => {
        if (annotation[m]) {
            row.push(transformValue_1.transformValue(dbRow[membersToAliasMap[m]], annotation[m].type));
        }
    });
    if (queryType === enums_1.QueryType.COMPARE_DATE_RANGE_QUERY) {
        ...
        console.log(dbRow[getBlendingResponseKey(timeDimensions)]);
...
function getVanilaRow(aliasToMemberNameMap, annotation, queryType, query, dbRow) {
    const row = ramda_1.default.pipe(ramda_1.default.toPairs, ramda_1.default.map(p => {
        ...
        const transformResult = [
            memberName,
            transformValue_1.transformValue(p[1], annotationForMember.type)
        ];
...
function transformData(aliasToMemberNameMap, annotation, data, query, queryType, resType) {
    const d = data;
    const membersToAliasMap = getMembers(queryType, query, d, aliasToMemberNameMap);
