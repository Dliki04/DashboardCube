...
class PreAggregations {
    ...
    static canUsePreAggregationForTransformedQueryFn(transformedQuery, refs) {
         ...
         * @type {Set<string>}
         */
        const filterDimensionsSingleValueEqual = transformedQuery.filterDimensionsSingleValueEqual instanceof Set
        ...
        const canUsePreAggregationNotAdditive = (references) => {
            const refTimeDimensions = references.sortedTimeDimensions ||
            ...
            return ((transformedQuery.hasNoTimeDimensionsWithoutGranularity) && (!transformedQuery.hasCumulativeMeasures) && (ramda_1.default.equals(qryTimeDimensions, refTimeDimensions)) && (transformedQuery.isAdditive ||
                ramda_1.default.equals(transformedQuery.timeDimensions, refTimeDimensions)) && (references.dimensions.length === filterDimensionsSingleValueEqual.size &&
                ramda_1.default.all(d => filterDimensionsSingleValueEqual.has(d), references.dimensions)) && (ramda_1.default.all(m => references.measures.indexOf(m) !== -1, transformedQuery.measures) ||
        ...
        const canUsePreAggregationLeafMeasureAdditive = (references) => {
            ...
            return ((windowGranularityMatches(references)) && (ramda_1.default.all(m => references.measures.indexOf(m) !== -1, transformedQuery.leafMeasures)) && (ramda_1.default.all(d => (references.sortedDimensions ||
                references.dimensions).indexOf(d) !== -1, transformedQuery.sortedDimensions)) && (ramda_1.default.allPass(queryTimeDimensionsList.map(tds => ramda_1.default.anyPass(tds.map(td => ramda_1.default.contains(td)))))(references.sortedTimeDimensions ||
                sortTimeDimensions(references.timeDimensions))));
        };
        /**
         * Determine whether pre-aggregation can be used or not.
